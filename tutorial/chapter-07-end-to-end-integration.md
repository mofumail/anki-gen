...
The following diagram shows which data artifacts exist at each step and where they flow:
...
This pattern — accumulating independent results and converging them late — is characteristic of pipeline architectures. The alternative would be a single mutable state object that each module modifies in place. The accumulative approach is preferred here because it makes the data dependencies visible in the function signatures: `build_flashcard(vocab_info, llm_output, audio_paths)` tells you exactly what it needs, while `process(state)` hides which fields each module reads and writes.
