...
A **deck** is a collection of cards used for organizing study sessions. Our tool puts all cards in a deck called "Japanese," configurable via `config.py`.
...
- `anki_connect.py` can be reused for other purposes — any code that needs to interact with Anki can import it independently of the flashcard formatting logic.
- The flashcard format can be changed without touching the delivery code, and vice versa.

The data flow between them is one-way and simple: `build_flashcard()` produces a dict, `add_note()` consumes it. The dict is the interface contract. As long as it contains `front`, `back`, and `audio_paths` with the expected structure, both modules can evolve independently.
